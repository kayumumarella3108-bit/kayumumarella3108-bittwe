...
import { Pencil, CheckCircle2, PieChart, BarChart3, Layers, Filter, FileText, Plus, ArrowUpRight, ShieldCheck, Activity, X, Trash2, Users, Search, Building2, Zap, Download, Printer } from 'lucide-react';
...
import { InputPelangganPenyulangModal } from '../modals/InputPelangganPenyulangModal';
...
interface MasterPelangganViewProps {
  ...
  onAddPenyulang?: (newData: any) => void;
}
...
export const MasterPelangganView: React.FC<MasterPelangganViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'penyulang' | 'section' | 'distribusi' | 'rekapitulasi'>('penyulang');
  ...
  const [inputEditItem, setInputEditItem] = useState<any>(null);
    ...
    <div className="space-y-6 pb-12">
      ...
      {activeTab === 'penyulang' && (
        <div className="space-y-4">
          ...
          {/* Table Toolbar */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl">
            <div className="flex items-center gap-3">
              <div className="w-1.5 h-6 bg-teal-600 rounded-full" />
              <h2 className="text-sm font-black text-slate-800 uppercase tracking-tight">
                Rekapitulasi Pelanggan per Penyulang
              </h2>
            </div>
            {canEdit && (
              <button
                onClick={() => {
                  setInputEditItem(null);
                  setIsInputModalOpen(true);
                }}
                className="px-4 py-2 bg-teal-700 hover:bg-teal-800 text-white rounded-xl text-xs font-black transition-all shadow-lg shadow-teal-700/20 flex items-center gap-2 uppercase tracking-widest"
              >
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-extrabold border-b border-slate-200">
                    <th className="py-3.5 px-4 text-[10px] uppercase tracking-widest">NO</th>
                    <th className="py-3.5 px-4 text-[10px] uppercase tracking-widest">NAMA PENYULANG & KODE ID</th>
                    <th className="py-3.5 px-4 text-center text-[10px] uppercase tracking-widest">JUMLAH PELANGGAN</th>
                    {canEdit && <th className="py-3.5 px-4 text-center text-[10px] uppercase tracking-widest">AKSI</th>}
                  </tr>
                </thead>
                        ...
                        <tr key={penyulang.id} className="hover:bg-teal-50/50 transition-colors">
                          ...
                          <td className="py-3.5 px-4">
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                              ...
                              <span className="text-[10px] text-slate-500 font-bold">{penyulang.unit || 'ULP Baguala'}</span>
                            </div>
                          </td>

                          <td className="py-3.5 px-4 text-center">
                            <div className="inline-flex flex-col items-center p-2.5 bg-emerald-50 rounded-2xl border border-emerald-100 min-w-[120px] shadow-sm">
                              <span className="text-xl font-black text-emerald-700 leading-none">
                                {jml.toLocaleString('id-ID')}
                              </span>
                              <span className="text-[9px] font-black text-emerald-600/60 uppercase mt-1.5 tracking-widest">Total Pelanggan</span>
                            </div>
                          </td>
                          ...
                          {canEdit && (
                            <td className="py-3.5 px-4 text-center whitespace-nowrap">
                              <button
                                ...
                                className="px-3 py-1.5 bg-white hover:bg-teal-600 text-teal-700 hover:text-white rounded-xl transition-all font-black text-[10px] flex items-center gap-1.5 mx-auto cursor-pointer border border-teal-200 shadow-sm uppercase tracking-widest"
                              >
      ...
      {/* TAB 4: REKAPITULASI (ULP, KODE UNIT, PENYULANG, JML PELANGGAN) */}
      {activeTab === 'rekapitulasi' && (
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-black text-slate-800 uppercase tracking-tight">Rekapitulasi Data Pelanggan</h2>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-0.5">Ringkasan per Unit Layanan & Feeder</p>
                </div>
              </div>
              <button className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-[10px] font-black transition-all uppercase tracking-widest border border-slate-200">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-slate-500 font-black text-[10px] uppercase tracking-widest border-b border-slate-200">
                    ...
                    <th className="py-4 px-6">Unit Layanan (ULP)</th>
                    <th className="py-4 px-6">Kode Unit</th>
                    <th className="py-4 px-6">Nama Penyulang</th>
                    <th className="py-4 px-6 text-right">Jumlah Pelanggan</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-bold">
                  {filteredPenyulang.map((p, idx) => (
                    <tr key={p.id} className="hover:bg-teal-50/30 transition-colors">
                      <td className="py-4 px-6 text-slate-400 font-mono text-[11px]">{idx + 1}</td>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-7 h-7 bg-teal-100 rounded-lg flex items-center justify-center">
                            <Building2 className="w-3.5 h-3.5 text-teal-700" />
                          </div>
                          <span className="text-slate-700 text-sm">{p.unit || 'ULP Baguala'}</span>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <span className="text-slate-500 text-xs font-mono">{p.kodeUnit || '54110'}</span>
                      </td>
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          <span className="text-slate-900 text-sm font-black uppercase">{p.namaPenyulang}</span>
                          <span className="text-[10px] text-slate-400 font-normal">ID: {p.kodeId}</span>
                        </div>
                      </td>
      ...
      {/* Input Modal Pelanggan per Penyulang */}
